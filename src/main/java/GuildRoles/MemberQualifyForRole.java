...
public class MemberQualifyForRole extends ListenerAdapter {
    ...
    public void onGuildMessageReceived(@NotNull GuildMessageReceivedEvent event) {
        ...
        String roles = event.getGuild().getRolesByName(message[3], true).get(0).getId();
        ...
        if ("!role".equals(message[0]) && "request".equals(message[1]) && event.getChannel().equals(channelName)) {
            ...
            event.getGuild().addRoleToMember(event.getChannel().getAsMention(),event.getGuild().getRoleById(roles)).queue();
